...
- **`.github/workflows/etl.yml`**  
  `python-project/` veya `k8s/python/` değişince: ETL image build (linux/amd64), ECR’e push, `kubectl apply -f k8s/python/cronjob-etl.yaml` ile CronJob güncelleniyor (bir sonraki çalıştırmada yeni image kullanılır).
